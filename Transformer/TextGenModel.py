...
class SelfAttention(nn.Module):
    def __init__(self, embed_dim, heads, dropout = 0) -> None:
        ...
        # self.value_downsample = nn.Linear(self.embed_dim, self.head_dim)
        # self.key_downsample = nn.Linear(self.embed_dim, self.head_dim)
        # self.query_downsample = nn.Linear(self.embed_dim, self.head_dim)
    ...
    def forward(self, values, keys, queries, src_mask):
        ...
        # keys = keys.reshape(keys.shape[0], keys.shape[2], keys.shape[1])
        # queries = queries.reshape(queries.shape[0], queries.shape[2], queries.shape[1])
        ...
        # values = self.values_embed(values)
        # keys = self.keys_embed(keys)
        # queries = self.queries_embed(queries)
